...
case $1 in 
    "-u" | "--up")
        #1) Network setup
        ./network.sh up createChannel -c secure-rider-driver -ca

        #2) Chaincode deployment
        ./network.sh deployCC -c secure-rider-driver -ccn mpc-app -ccp ../MPC-Sample/chaincode-go -ccl go
        ...
        #4) Moving to app dir 
        cd ../MPC-Sample/application-gateway-go/
